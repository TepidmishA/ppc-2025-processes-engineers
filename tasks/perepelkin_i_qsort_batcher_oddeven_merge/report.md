...
В качестве последовательного алгоритма сортировки используется стандартная библиотечная функция `std::ranges::sort`, реализующая эффективный алгоритм сортировки общего назначения. Данная реализация применяется в работе в качестве базового варианта для сравнения с параллельной версией алгоритма.
...
- **Локальные сортировки:** Используется стандартная библиотечная функция `std::ranges::sort`.
...
| ----- | -------- | -------- | --------- | ---------------- |
| seq   | 1        | 1.119676 | 1.000     | N/A              |
| mpi   | 2        | 0.728798 | 1.536     | 76.82            |
| mpi   | 4        | 0.501945 | 2.231     | 55.77            |

**Результаты вычислительной части:**
| Режим | Процессы | Время, с | Ускорение | Эффективность, % |
| ----- | -------- | -------- | --------- | ---------------- |
| seq   | 1        | 1.112753 | 1.000     | N/A              |
| mpi   | 2        | 0.726808 | 1.531     | 76.55            |
| mpi   | 4        | 0.498068 | 2.234     | 55.85            |

**Анализ результатов (на основе сценария task_run):**
- **Производительность:** MPI-реализация демонстрирует ускорение относительно последовательной версии (ускорение 1.53x на 2 процессах и 2.23x на 4 процессах).
- **Эффективность параллелизации:** На 2 процессах эффективность составляет 76.55% с последующим снижением до 55.85% при использовании 4 процессов.
...
- Параллельная реализация алгоритма показывает увеличение производительности относительно последовательной версии: ускорение составляет 1.53x при использовании 2 процессов и 2.23x при 4 процессах.
- Эффективность параллелизации составляет 76.55% на 2 процессах с последующим снижением до 55.85% на 4 процессах.
