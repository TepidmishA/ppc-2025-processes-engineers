...
В работе представлена реализация данного алгоритма с использованием технологии MPI и сравнительный анализ её производительности с последовательной версией при различном количестве процессов для оценки эффективности распараллеливания.

## 2. Постановка задачи
**Цель работы:**\
Разработка параллельной MPI-реализации алгоритма подсчёта несовпадающих символов и сравнительный анализ её производительности с последовательной версией при различном количестве процессов.
...
- Анализ работы алгоритма на граничных случаях:
  ...
  - Поддержка многобайтовыми символов UTF-8.
...
**Анализ результатов:**
- **Высокая эффективность параллелизации** – на 2 процессах достигается 92% эффективности, что демонстрирует близкое к линейному ускорение.
- **Снижение эффективности при масштабировании** – при увеличении числа процессов до 4 эффективность падает до 75%, что связано с ростом коммуникационных затрат.
- **Ограничения архитектуры** – необходимость хранения полных копий строк в каждом процессе создает повышенные требования к оперативной памяти.
...
- Достигнуто близкое к линейному ускорение при использовании 2 процессов (эффективность 92%).
- При масштабировании до 4 процессов сохраняется значительное ускорение (3.02 раза) при снижении эффективности до 75%.

**Ограничения:**
- Основное ограничение – необходимость хранения полных копий строк в каждом процессе.
- Коммуникационные затраты операции MPI_Allreduce снижают эффективность при увеличении числа процессов.
