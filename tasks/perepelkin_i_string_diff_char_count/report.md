...
- **Распределение данных:** Процесс 0 распределяет сегменты данных между всеми процессами с помощью `MPI_Scatterv` с балансировкой нагрузки – первые `remainder` процессов получают один дополнительный элемент при неравномерном делении.
